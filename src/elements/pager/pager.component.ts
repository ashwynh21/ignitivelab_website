  ...
  prime(): void {
  	...
  	const hookes = () => {
         ...
         */
  			...
  			if (axis === 'x') {
  				if (target.px <= margin.left) {
  					result += margin.left - target.px;
  				} else if (target.px >= (margin.right - padding.width)) {
  					result += ((margin.right - padding.width) - target.px);
  				}

  				return result;
  			} else {
  				if (target.py <= margin.top) {
  					result += margin.top - target.py;
  				} else if (target.py >= (margin.bottom - padding.height)) {
  					result += ((margin.bottom - padding.height) - target.py);
  				}
